...
public class UserActivity {
	...
	public void deleteQuestion(UserActivity userActivity, User user) throws NumberFormatException, IOException {
		...
		// Write else if condition to check if the user is a moderator
            ...
            // the user who wrote the question is a regular user
            if(question.getUser().getUserRole() == UserRole.USER){
                questionService.deleteQuestion(question);
            }
            // current user had written the question which needs to be deleted
            else if(user == question.getUser()){
                questionService.deleteQuestion(question);
            }
            // if no conditions mentioned are satisfied for a user with moderator role,
            // the user cannot delete the question
            else{
